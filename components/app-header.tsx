...
export function AppHeader({ user, onLogout, currentPage }: AppHeaderProps) {
  ...
  if (user.role === "ADMIN") {
    navigationItems = [
      { href: "/dashboard", label: "Accueil", icon: LayoutDashboard, color: "indigo", active: currentPage === "dashboard" },
      { href: "/admin//departements", label: "Départements", icon: Building, color: "blue", active: currentPage === "departements" },
      { href: "/admin/niveaux", label: "Niveaux", icon: GraduationCap, color: "purple", active: currentPage === "niveaux" },
      { href: "/admin/matieres", label: "Matières", icon: BookOpen, color: "emerald", active: currentPage === "matieres" },
      { href: "/admin/salles", label: "Salles", icon: Building2, color: "orange", active: currentPage === "salles" },
      { href: "/admin/seances", label: "Séances", icon: CalendarCheck, color: "yellow", active: currentPage === "seances" },
      { href: "/emploi-du-temps", label: "EDT", icon: Grid3X3, color: "blue", active: currentPage === "emploi-du-temps" },
      { href: "/admin/users", label: "Utilisateurs", icon: Users, color: "green", active: currentPage === "users" },
    ]
  } else if (user.role === "ETUDIANT") {
    navigationItems = [
      { href: "/dashboard", label: "Accueil", icon: LayoutDashboard, color: "blue", active: currentPage === "dashboard" },
      { href: "/emploi-du-temps", label: "Mon EDT", icon: CalendarCheck, color: "blue", active: currentPage === "emploi-du-temps" },
      { href: "/etudiant/grades", label: "Mes Résultats", icon: BookOpen, color: "purple", active: currentPage === "grades" },
    ]
  } else if (user.role === "ENSEIGNANT") {
    navigationItems = [
      { href: "/dashboard", label: "Accueil", icon: LayoutDashboard, color: "emerald", active: currentPage === "dashboard" },
      { href: "/emploi-du-temps", label: "Séances", icon: CalendarCheck, color: "emerald", active: currentPage === "emploi-du-temps" },
          ...
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-4 bg-gray-50/80 rounded-xl p-4 pr-6 hover:bg-gray-100/80 transition-all duration-200 cursor-pointer">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-sm text-gray-600">{user.role === "ADMIN" ? "Administrateur" : user.role === "ETUDIANT" ? "Étudiant" : "Enseignant"}</p>
              </div>
            </div>
