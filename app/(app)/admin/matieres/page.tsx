...
import {
  ...
  CheckCircle,
  Search,
  GraduationCap, // Pour les niveaux
  Building,       // Pour les départements
} from "lucide-react"
...
export default function MatieresPage() {
  ...
  const [matiereToDeleteId, setMatiereToDeleteId] = useState<string | null>(null)
    ...
    if (storedUser) {
      try {
        ...
        if (parsedUser.role !== Role.ADMIN) {
          setError("Accès refusé. Seuls les administrateurs peuvent gérer les matières.")
        }
      } catch (e) {
  ...
  const fetchMatieres = useCallback(async () => {
    ...
    // Charger les niveaux d'abord car ils sont nécessaires pour les matières
  ...
  // Déclencher le chargement initial des matières
  useEffect(() => {
    if (user && user.role === Role.ADMIN) {
      fetchMatieres()
    }
    ...
    if (searchTerm.trim() === "") {
      ...
      const filtered = matieres.filter(
        (matiere) =>
          ...
          matiere.niveau?.departement?.nom.toLowerCase().includes(searchTerm.toLowerCase()),
      )
  ...
  const handleOpenEditModal = (matiere: Matiere) => {
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    if (!newMatiereName.trim()) {
      toast.error("Le nom de la matière ne peut pas être vide.", { duration: 3000 })
      return
    }
    if (!selectedNiveauId) {
      toast.error("Veuillez sélectionner un niveau.", { duration: 3000 })
      return
    }
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ nom: newMatiereName, niveauId: selectedNiveauId }),
      })
      ...
      toast.success(`Matière ${currentMatiere ? "mise à jour" : "créée"} avec succès.`, { duration: 3000 })
      handleCloseFormModal()
      fetchMatieres() // Recharger les matières
    } catch (err: any) {
      console.error("Erreur lors de l'opération sur la matière:", err)
  ...
  const handleOpenDeleteDialog = (id: string) => {
    setMatiereToDeleteId(id)
    setIsDeleteDialogOpen(true)
  }

  const handleDeleteConfirm = async () => {
    if (!matiereToDeleteId) return
    ...
    try {
      ...
      toast.success("Matière supprimée avec succès.", { duration: 3000 })
      setIsDeleteDialogOpen(false)
      setMatiereToDeleteId(null)
      fetchMatieres() // Recharger les matières
    } catch (err: any) {
  ...
  const totalMatieresCount = matieres.length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50/30 to-purple-50/20 relative overflow-hidden">
      ...
      <div className="p-6 lg:p-8">
        ...
        >
                  ...
                  <CardContent className="p-6 relative z-10">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-blue-600 font-medium">Matières Actives</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          {/* Section principale - Liste des matières */}
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-gray-600 text-base font-medium">
                      Gérez toutes les matières enseignées
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="p-0 relative z-10">
                {loading ? (
                  <div className="flex flex-col items-center justify-center p-16">
                    ...
                    <p className="text-gray-600 text-xl font-semibold">Chargement des matières...</p>
                  </div>
                  ...
                  <div className="text-center p-16">
                    ...
                    <h3 className="text-2xl font-bold text-gray-700 mb-4">
                      {searchTerm ? "Aucun résultat trouvé" : "Aucune matière"}
                    </h3>
                            ...
                            >
                              {/* <TableCell className="py-6 px-6 font-mono text-sm text-gray-500 w-32">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="truncate">{matiere.id.substring(0, 8)}...</span>
                                </div>
                              </TableCell> */}
                              ...
                              <TableCell className="py-6 px-6">
                                <div className="flex items-center space-x-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-gray-500 text-sm">Matière scolaire</p>
                                  </div>
                                </div>
                              </TableCell>
                                ...
                                <div className="flex items-center space-x-2">
                                  <motion.div whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}>
                                    ...
                                    >
                                      ...
                                      {matiere.niveau ? matiere.niveau.nom : "N/A"}
                                    </Badge>
                                  </motion.div>
                                </div>
                              </TableCell>
                                ...
                                <div className="flex items-center space-x-2">
                                  <motion.div whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}>
                                    ...
                                    >
                                      ...
                                      {matiere.niveau?.departement ? matiere.niveau.departement.nom : "N/A"}
                                    </Badge>
                                  </motion.div>
                                </div>
                              </TableCell>
                                ...
                                <div className="flex items-center justify-center space-x-2">
                                  ...
                                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                                    <Button
                                      variant="destructive"
                                      size="sm"
                                      onClick={() => handleOpenDeleteDialog(matiere.id)}
                ...
                <form onSubmit={handleSubmitForm} className="p-6 space-y-8 relative z-10">
                  <div className="space-y-6">
                    <div className="space-y-3">
                      <Label htmlFor="matiereName" className="text-lg font-bold text-gray-700 flex items-center">
                        <BookOpen className="h-5 w-5 mr-2 text-blue-600" />
                        Nom de la matière
                      </Label>
                      <div className="relative group">
                        <BookOpen className="absolute left-4 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400 group-focus-within:text-blue-500 transition-colors duration-300" />
                        <Input
                          id="matiereName"
                    ...
                    <div className="space-y-3">
                      <Label htmlFor="niveauSelect" className="text-lg font-bold text-gray-700 flex items-center">
                        <GraduationCap className="h-5 w-5 mr-2 text-purple-600" />
                        Niveau
                      </Label>
                      <div className="relative group">
                        <Select onValueChange={setSelectedNiveauId} value={selectedNiveauId}>
                            ...
                            <SelectValue placeholder="Sélectionner un niveau">
                              {selectedNiveauId ? (
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span>{niveaux.find(niv => niv.id === selectedNiveauId)?.nom} ({niveaux.find(niv => niv.id === selectedNiveauId)?.departement.nom})</span>
                                </div>
                              ) : (
                                <div className="flex items-center space-x-2 text-gray-500">
                                  <GraduationCap className="h-5 w-5 text-gray-400" />
                                  <span>Sélectionner un niveau</span>
                                </div>
                              )}
                            </SelectValue>
                          </SelectTrigger>
                          <SelectContent className="rounded-2xl border-2 border-gray-200/50 shadow-2xl backdrop-blur-xl bg-white/95">
                            {niveaux.length === 0 ? (
                              <SelectItem value="no-niveaux" disabled className="text-gray-500 italic">
                              ...
                              niveaux.map((niv) => (
                                ...
                                >
                                  <div className="flex items-center space-x-2">
                                    ...
                                    <span>{niv.nom} ({niv.departement.nom})</span>
                                  </div>
                                </SelectItem>
                              ))
                            )}
                      ...
                      >
                        ...
                        {isSubmitting ? (
                          <span className="flex items-center justify-center relative z-10">
                            ...
                            {currentMatiere ? "Modification..." : "Création..."}
                          </span>
                        ) : (
                          <span className="flex items-center justify-center relative z-10">
                            {currentMatiere ? (
                ...
                <div className="bg-gradient-to-r from-red-600 to-rose-600 p-8 text-white">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-red-100 text-lg mt-4">
                      Êtes-vous sûr de vouloir supprimer définitivement cette matière ? Cette action est irréversible et
                      toutes les données liées (séances, etc.) pourraient être affectées.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                </div>
